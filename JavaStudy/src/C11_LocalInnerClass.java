...
public class C11_LocalInnerClass {
	/*
	 	...
	 	- 이름을 지정할 수 없기 때문에 익명 클래스 라고 불린다
	*/
...
//		Student stu = new Student();
	...
	public static void main(String[] args) {
		...
		doSomething2(new Book() {
			@Override
			public void print() {
				System.out.println("익명의 클래스에서 덮어쓴 프린드입니다.");
			}
		});
		...
		class Comicbook extends Book {
			@Override
			public void print() {
				System.out.println("부모의 기능을 덮어써서 내맘대로 바꿨음");
			}
		}
		
		// 부모타입을 원하는 메서드에 부모 타입 잇ㄴ스턴스를 전달
		doSomething2(new Book());
		// 부모타입을 필요로하는 매개변수에 자식 타입을 전달 (다형성)
		doSomething2(new Comicbook());
		// 부모타입을 필요로 하는 매개변수에 바로 상속 받은 익명(이름없는 타입)을 전달 (다형성)
		doSomething2(new Book() {
			public void print() {
				System.out.println("Book을 상속받아서 기능을 맘대로 고쳐썼지만"
