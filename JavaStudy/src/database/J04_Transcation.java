...
public class J04_Transcation {
	...
	public static void main(String[] args) {
		...
		try (
			...
			// 트랜젝션을 직접 제어하기 위해 오토커밋을 비활성화
			conn.setAutoCommit(false);
			
			// 1. 판매가 발생하면 매출액이 증가한다
			// 	  (오늘의 매출이 조회가 되지 않는 상태(COUNT = 0)라면 새로운 행을 0으로 생성한다)
			...
			try(
				...
				System.out.printf("%d행 변경완료\n", row);
			} catch(SQLException e) {
				e.printStackTrace(); // 스택찍고
				conn.rollback(); // 롤백하고
				return; // 메서드 강제종료
			}
			...
			// 2. 해당 과일의 재고를 감소 시킨다
			String reduceQty = "UPDATE fruits SET qty = qty - ? "
					+ "WHERE fruit_name = ?";
