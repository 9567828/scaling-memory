...
public class B00_Scanner {
	...
	/*
 		...
 		System.in : 콘솔에서부터 프로그램으로 데이터가 들어오는 통로
 		
 		# 스캐너 클래스의 대표적인 기능들
 		
 		 (1) next타입() 메서드 - nextInt(), nextDouble(), nextFloat() ...
 		 
 		  - 입력 통로로 부터 다음 값 하나를 꺼내오는 기능
 		  - 문자열 형태로 값을 꺼낸 후 원하는 타입으로 변환해준다
 		  	입력값 > 123 (type of String) > 변환 > type of Number 123
 		  - 입력통로의 값들은 공백, \t(탭), \n(엔터)를 기준으로 구분할 수 있다
 		  - 입력통로에 꺼낼 값이 없는 경우 프로그램을 멈추고 새로운 값을 기다린다
 		  - ※ nextString()은 없다
 		 ...
 		 (2) next() 메서트
 		  ...
 		  - 입력 통로로부터 다음 값 하나를 문자열로 꺼내는 기능
 		    (무엇이든 입력 가능)
 		  - 입력 통로의 값들은 공백, \t, \n을 기준으로 구분하기 때문에 문장은 입력 받을 수 없다
 		  
 		 (3) nextLine() 메서드
 		 
 		  - 입력 통로로부터 다음 값 하나를 문자열로 꺼내는 기능
 		  - \n만을 기준으로 구분하기 때문에 한 줄 전체를 입력 받을 수 있다
 		  	(공백, \t 취급안한다)
	...
	*/
	...
	public static void main(String[] args) {
		...
		// 새 스캐너 생성(NEW)과 동시에 어디에서 입력 받을 지 설정
		// System > 각종 프로그램이 될 수 있다
		...
		// 				  (다음 정수 입력을 기다리는 기능)
		//				  (괄호안에 1 2 3 4 같은 형태로 여러숫자를 넣을 수 있다)
		int num = sc.nextInt();
