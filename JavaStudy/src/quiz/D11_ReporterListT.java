...
public class D11_ReporterListT {
	
	 /*
 		1. 어떤 이메일 도메인(yna.co.kr, hankyung.com) 이 몇 개씩 있는지 세어서 출력해보기
 		2. 어떤 신문사의 기자가 몇 명씩 있는 지 세어서 출력
 		3. 어떤 분야의 기자가 몇 명씩 있는 지 세어서 출력
 		4. 모든 기자의 이름/이메일/분야를 신문사별로 분류하여 출력
 		5. 모든 기자의 이름/이메일/신문사를 분야별로 분류하여 출력
	*/
	...
	public static void main(String[] args) {
		...
		while(m.find()) {
			...
			System.out.println("이메일이아디:\t" + m.group(5));
...
//			}
		...
		System.out.println("# 각 분야의 기자수");
		System.out.println(fieldCountMap);
		
		// HashMap - 키값과 밸류를 넣어준다
		// 키값 = 회사의 이름 / 벨류는 List로 제네릭은 Reporter로 받아주는 것
		// 리스트는 위에 ArrayList로 생성 되어잇는 것을 넣은 것
		HashMap<String, List<Reporter>> categoryCompany = new HashMap<>();
		HashMap<String, List<Reporter>> categoryField = new HashMap<>();
		
		System.out.println("#### 신문사별 ####");
		System.out.println();
		
		
		// 정규 표현식으로 찾은 기자들을 모두 더해놓은 리스트
		// forEach 하나씩 꺼내면서 반복
		for (Reporter reporter : reporters) {
			// 기자를 한 명씩 꺼내면서 해당 기자의 회사 이름을 꺼내온다
			String companyName = reporter.getCompnay();
			// 카테고리컴퍼니에 겟컴퍼니를 넣은 컴퍼니네임을 containsKey메소드로 이름이 있는지 확인한다
			// true면 카테고리컴퍼니에 컴퍼니네임을 get하고 reporter를 add하는 것
			if (categoryCompany.containsKey(companyName)) {
				// add 리포터는 새 인스턴스가 생기는 것이 아니라 for문에 report를 넣는 것이다
				categoryCompany.get(companyName).add(reporter);
//				아래 문법을 위 처럼 한 줄로
//				List<Reporter> list = categoryCompany.get(companyName);
//				list.add(reporter);
				
			} else {
				// if가 아니면 list를 생성해서 인스턴스를 값으로 넣어라 reporter를 add해주고 카테고리캄파니에 put을 해줘라
