...
public class D12_Date {
	...
	*/
	...
	public static void main(String[] args) {
		...
		// ※ Unix Time - 1970년 1월 1일 오전 9시 이후로 시간이 얼마나 흘렀는지 밀리초(1/1000) 세는 것
		System.out.println("1970년 이후로 흐른 시간: " + System.currentTimeMillis() / 1000 + " 초");
		
		// 원하는 날짜로 시간을 생성하는 기능이 있지만 더이상의 업데이트를 해주지 않는다는 의미로 작대기가 나옴
		// 지원 중단
		...
		System.out.println(onceOpenAtime);
		
		// date.after(anotherDate)
		// date.before(anotherDate)
		// : 두 시점을 비교해 어떤 시점이 더 뒤인지 알 수 있다
		System.out.println("now가 원스어폰어타임보다 이후 시점?? " + now.after(onceOpenAtime));
		System.out.println("now가 원스어폰어타임보다 이전 시점?? " + now.before(onceOpenAtime));
		
		// getTime() : 인스턴스로부터 해당 시간의 유닉스 타임을 꺼낼 수 있다
		System.out.println(now.getTime());
		System.out.println(onceOpenAtime.getTime());
		
		// setTime(unixTime) : 유닉스 타임을 전달해 인스턴스의 시간은 변경한다
		now.setTime(10000000L);
		System.out.println(now);
		
		// 다양한 getter들이 존재하지만 모두 지원 중단 되어서 별로 사용되지 않는다
		now.getYear();
		now.getMonth();
		now.getHours();
		now.getDay();
