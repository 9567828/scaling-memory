...
public class E12_NetworkProgramming {
	...
	/*
 		...
 		- 서버소켓에 소켓이 접속 되고 나면 양방향으로 통신이 가능하다
 		- 서로 원하는 데이터를 전송할 수 있다
 		- 상대방에게 전송하는 것이 Output이고 상대방으로부터 받은 것이 Input
 		
 		# 네트워크 기본지식
 		
 		- IP주소 : 0.0.0.0 ~ 255.255.255.255 사이의 번호로 이루어진 컴퓨터의 주소
 		  ※ IP주소는 cmd > ipconfig 명령어로 확인할 수 있다
 		- 프로토콜(Protocol) : 통신규칙 (TCP, UDP, SMTP(이메일), Telnet, ftp(파일전송), http...)
 		  랜선을 통해서 통신을 받을 때 규칙이 정해진다는 것 (0101010 .. 사이에 규칙이 정해져 있다)
 		- 포트번호: 하나의 컴퓨터에서 각 프로그램을 구분하기 위한 용도로 사용되는 번호
 		  ※ 현재 컴퓨터에서 열려있는 포트 번호는 cmd -> netstat 명령어로 확인할 수 있다
 		
 		# Socket
 		
 		- 두 컴퓨터가 통신을 하기위해 사용하는 클래스
	...
	*/
	...
	public static void main(String[] args) {
		...
		try (
			...
			int cnt = 0;
			while(true) {
				// 다른 소켓이 연결될 때 까지 프로그램을 멈추고 기다린다..
				// 연결되고 나면 연결 정보를 담은 소켓을 리턴한다
				System.out.println("다음 연결을 기다리는 중...");
				Socket s = ss.accept();
				
				// 서버에게 연결된 상대방은 클라이언트라고 한다
				// 연결된 상대방(클라이언트)에게 보낼 수 있는 통로를 떠내온다
				try (
