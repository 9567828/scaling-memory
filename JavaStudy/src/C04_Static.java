...
public class C04_Static {
	...
	/*
		...
		- static 영역의 자원은 인스턴스 이름이 아닌 클래스 이름으로 접근할 수 있다
		- 스태틱 영역은 인스턴스가 하나도 생성되지 않은 시점에도 존재한다
		  (최초로 해당 클래스가 호출되었을 때 생성된다)
		- 스태틱 영역의 값은 인스턴스 영역(변수)에 대입(활용)할 수 있지만
		  인스턴스 영역(변수)의 값은 스태틱 영역에 활용할 수 없다
		  
		# static을 붙일 수 있는 자원
		
		- static 변수 (모든 인스턴스에서 항상 같은 값을 가지는 공동의 변수)
		- static 메서드 (인스턴스가 존재하지 않을때도 동작할 수 있는 메서드가 된다)
		- static 클래스 내부의 클래스 (? 나중에 설명)
	*/
	...
	public static void main(String[] args) {
		...
		System.out.println("스페이드A의 너비: " + heart_10.rank);
		System.out.println("스페이드A의 너비: " + heart_10.suit);
...
class TrumphCard {
	...
	int rank; // A~K
	int suit; // 스페이드0, 하트1, 다이아몬드2, 클로버3
	
	// 카드의 너비와 높이는 모든 인스턴스에서 다를 필요가 없는 값이다.
	// 이런 인스턴스 변수값이 낭비되는 영역이다
	// 이런 성질의 변수를 인스턴스 값으로 두면 메모리의 공간이 낭비된다
	// 다른 변수와 달리 모든 영역에 적용 된다
...
// myobj 패키지에 static 자원을 포함한 클래스를 하나 설계해 보세요