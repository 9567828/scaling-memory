...
public class A02_Variable {
	...
	/*
		...
		# 메모리 (RAM, 주기억장치, 전기적장치-빠름)
		
		- 전기가 통하는 동안 데이터가 계속 유지되는 성질을 가지고 있는 저장 매체
		- 현재 진행상황을 보관해놓는 용도로 사용하는 기억 장치
		
		# 하드드라이브 (보조기억장치 (USB, CD룸, 플로피디스켓등), 물리적장치-느림)
		  ※ SSD는 전기장치라서 빠르다 (하이브리드, 뉴메타)
		
		- 전기가 통하지 않더라도 데이터를 계속해서 보관해주는 성질을 가지고 있는 저장 매체
		- 주로 어떤 프로그램으로 '저장하기' 기능을 수행했을 때 
		  보조 기억장치에 해당 데이터를 보관한다
		  
		▶ 하드드라이브에 저장 되어있는 프로그램을 실행할 때 메모리에서 받아서 CPU로 보낸다
		  해서 RAM이 가득차면 컴퓨터가 느려지는 것
		
		# 변수의 선언 (declare)
			
		  타입 변수명;
		
		- 최초로 해당 변수에 저장할 테이터의 타입과 변수의 이름을 정하는 것
		- 변수에 저장할 데이터의 종류에 따라 빌려오는 메모리의 크기가 다르다
		
		# 대입 연산 (=)
		
		- 원래 알고있던 =의 의미: 왼쪽의 값과 오른쪽의 값이 같다
		- 프로그래밍 언어에서 =의 의미: 왼쪽의 변수에 오른쪽 값을 넣어라 (대입)
		
	*/

	// 만약 main() 함수에 틀린점이 있다면 가장 최근에 성공했던 main을 대신 실행한다 ex. mian 등 오타
	public static void main(String[] args) {
		...
		// 3 = x; < 왼쪽에 있는 변수에 오른쪽 값을 넣어야 하기 때문에 안됨.
		...
		//  ※ 변수를 선언한 후에 값을 한 번도 대입하지 않은 경우 해당 변수를 활용할 수 없다
		...
		System.out.println("사용자 점수는 " + user_score + "이고, 컴퓨터 점수는 " + + com_score + "입니다");
		System.out.println("현재 설정된 환영 인사: " + welcome);
		System.out.println("현재 설정된 퇴장 인사: " + goodbye);
		
		// 4. 한 번에 변수 여러개 선언과 동시에 대입
		...
		// ※ 같은 {} 내부에서 위에서 사용했던 변수명으로 다시 사용할 수 없다. (같은 이름 사용x)
