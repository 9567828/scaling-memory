...
public class A04_VariableNaming {
	...
	/*
		...
		3. 변수명 첫 글자는 알파벳 소문자를 사용해야 한다
		   ...
		   변수명 첫 글자는 알파벳 소문자를 사용해야 한다
		   (첫글자가 대문자인 경우는 암묵적으로 클래스로 간주한다)
		   
		   ex. Apple (클래스로 간주), apple(변수로 간주)
		   
		4. 프로그램이 종료 될 때까지 값이 절대로 변하지 않을 값은 모두 대문자를 사용한다
		   (상수를 저장하는 변수명은 모두 대문자로 표기한다)
		   
		   ex. PI, APPLE_PRICE, WIN_SHODRE
		   
		5. 패키지 명은 모두 소문자를 사용한다
		   ex. Quiz(클래스) quiz(패키지)
		
		6. 해당 변수에 들어있는 값이 무엇인지 쉽게 파악할 수 있는 이름을 사용해야 한다
		   ex. a1, a2,a3,.....(x);
		   
		# 변수의 작명규칙 (필수사항, 지키지 않으면 컴파일 불가능)
		
		1. 변수명의 첫글자에는 숫자를 사용할 수 없다.
		   ex. 1st, 2nd, 3rd(x)
		
		2. 변수명 사이에는 공백을 사용할 수 없다.
		
		3. 특수 문자는 _와 $만 사용할 수 있따
		   ex. $spuer, _apple(O)
		
		4. 이미 자바에서 사용하고 잇는 키워드들은 변수명으로 사용할 수 없다
	...
	*/
	...
	public static void main(String[] args) {
		...
		System.out.println("사과의 개당가격: " + apple_price + "원");
