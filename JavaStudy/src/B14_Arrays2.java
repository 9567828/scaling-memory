...
public class B14_Arrays2 {
	...
	*/
	...
	public static void main(String[] args) {
		...
		// int[]을 요소로 가지는 int[][] - 2차형 배열 이라고도 한다
		int[][] arr5 = {{1,2,3},{3,3,3,3},{5,5,5,5},arr1,arr2,arr3,arr4};
		...
		// 재미있는 점
		// 배열의 변수명으로 값을 전달하면 만들어진 실체의 위치를 전달하는 것이다
		// 아래 에제에서 자바 클래스의 실체는 하나 이기 때문에
		...
		for (int i = 0; i < scores1.length; ++i) {
			total = scores1[0] + scores1[i];
//			System.out.println(Arrays.toString(scores1));
		}
		System.out.print(total);
		
		System.out.println();
		// 엔차원 배열은 n중 반복문으로 모든요소를 탐색할 수 있다
		...
		System.out.println(doll[1][2] = '○');
		
		System.out.println(Arrays.toString(doll));
		...
		// <연습> : messages에 포함된 모든 요소를 반복문으로 한 글자씩 출력해보세요
